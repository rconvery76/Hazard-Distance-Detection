...
def set_camera_parameters():
    ...
    pitch_deg = math.radians(-50.0)  # -70 => 70 up from horizontal = 20 downfrom vertical
...
def add_distance_to_detections_img(detection_results, bounding_box_img, boxes):
    for key in detection_results.keys():
        _, Yw = detection_results[key]['world_coordinates']
        distance_inches = abs(Yw) * 39.3701  # convert meters to inches
        # Find corresponding box to get position for text
        index = int(key.split('_')[-1])
        box = boxes[index]
        x1, y1, x2, y2 = box
        x_center = (x1 + x2) / 2.5
        y_center = (y1 + y2) / 2.0
        # Position text in the bounding box
        text_position = (int(x_center), int(y_center))
        cv2.putText(bounding_box_img, f"{distance_inches:.1f} in", text_position,
                    cv2.FONT_HERSHEY_SIMPLEX, 3, (0, 0, 255), 5)
        
    #save the output image with distances
    output_with_distances = "OUTPUT/detections_with_distances.png"
    cv2.imwrite(output_with_distances, bounding_box_img)
...
if __name__ == "__main__":
    ...
    add_distance_to_detections_img(detection_results, detected_image, boxes)
